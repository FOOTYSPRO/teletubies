...
const LIQUIDITY = 500; // Misma constante de estabilidad
...
export default function TorneoPage() {
  ...
  // ... (Funciones de crear torneo IGUALES que antes) ...
  ...
  const handleCrearTorneo = async () => { /* ... Código de crear torneo ... */ 
  ...
  const finalizarPartido = async (matchId: number, s1: number, s2: number) => {
    ...
    try {
      ...
      // 1. CÁLCULO DE POOLS (Simple Bets)
      ...
      pending.forEach((b: any) => {
          ...
          if (won) {
              let odd = 1.0;
              // Si es combinada, usamos su cuota fija. Si es simple, calculamos con la formula estabilizada
              if (b.type === 'combined') {
      ...
      // RANKING Y AVANCE (Sin cambios)
      // ... (Omito código repetido de avanzar ronda para no hacer el mensaje eterno)
      // ... Pega aquí la lógica de propagate y setDoc del mensaje anterior ...
      // Para asegurar que funciona, incluiré la lógica mínima de avance:
      let next = [...matches];
      ...
      const isP1_2 = s1 > s2; // Helper
      const isSmall = matches.length === 4;
      if (isSmall) { if(matchId===0){send(2,'p1',winner,isP1_2?m.p1Team:m.p2Team,isP1_2?m.p1Club:m.p2Club);send(3,'p1',isP1_2?m.p2:m.p1,isP1_2?m.p2Team:m.p1Team,isP1_2?m.p2Club:m.p1Club);} if(matchId===1){send(2,'p2',winner,isP1_2?m.p1Team:m.p2Team,isP1_2?m.p1Club:m.p2Club);send(3,'p2',isP1_2?m.p2:m.p1,isP1_2?m.p2Team:m.p1Team,isP1_2?m.p2Club:m.p1Club);} } 
      else { if(matchId<=3) send(matchId < 2 ? 4 : 5, matchId % 2 === 0 ? 'p1' : 'p2', winner, isP1_2?m.p1Team:m.p2Team, isP1_2?m.p1Club:m.p2Club); if(matchId===4){send(6,'p1',winner,isP1_2?m.p1Team:m.p2Team,isP1_2?m.p1Club:m.p2Club);send(7,'p1',isP1_2?m.p2:m.p1,isP1_2?m.p2Team:m.p1Team,isP1_2?m.p2Club:m.p1Club);} if(matchId===5){send(6,'p2',winner,isP1_2?m.p1Team:m.p2Team,isP1_2?m.p1Club:m.p2Club);send(7,'p2',isP1_2?m.p2:m.p1,isP1_2?m.p2Team:m.p1Team,isP1_2?m.p2Club:m.p1Club);} }

      await setDoc(doc(db, "sala", "principal"), { fifaMatches: next }, { merge: true });
      if (!winner.includes("CPU") && !m.isBye) {
         if (winner.includes(" & ")) { const [p1, p2] = winner.split(" & "); if(!p1.includes("CPU")) setDoc(doc(db, "ranking", p1), { puntos: increment(3), victorias: increment(1) }, { merge: true }); if(!p2.includes("CPU")) setDoc(doc(db, "ranking", p2), { puntos: increment(3), victorias: increment(1) }, { merge: true }); }
         else setDoc(doc(db, "ranking", winner), { puntos: increment(3), victorias: increment(1) }, { merge: true });
      }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-6">
        ...
        {matches.length === 0 ? (
            ...
            <div className="grid gap-8"><div className="space-y-4"><h3 className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Partidos</h3>{(matches.length===4 ? [0,1,2,3] : [0,1,2,3,4,5,6,7]).map(id => matches[id] && <MatchCard key={id} m={matches[id]} onFinish={finalizarPartido} isFinal={id===2 || id===6} />)}</div></div>
        )}
    </div>
  );
}
